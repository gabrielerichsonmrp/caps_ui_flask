...
def index():
    ...
    stats = {
        ...
        # Silahkan melakukan agregasi data yang tepat menggunakanse groupby untuk menampilkan 10 aplikasi yang diurutkan berdasarkan 
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews':'sum','Rating':'mean'}).sort_values(ascending=False, by='Reviews').head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    'Category' : 'count'
        }).rename({'Category':'Total'}, axis=1).sort_values(ascending=False, by='Total').head()
    X = cat_order.index.to_list()
    Y = cat_order['Total'].to_list()
    my_colors = 'rgbkymc'
    ...
    plt.barh(X,Y, color=my_colors) # Masukan X dan Y
    ...
    # Menampilkan proporsi jumlah aplikasi free dan paid dari 10 kategori yang paling popular
    top_install= df2.groupby(['Category']).agg({'App':'count'}).sort_values(ascending=False, by=['App']).head(10).index.to_list()
    top_install_category = df2[df2['Category'].isin(top_install)].groupby(['Category','Type']).agg({'App':'count'}).dropna().unstack().reset_index()
    top_install_category = top_install_category[top_install_category['App']['Free']>0]  
    category=top_install_category['Category'].to_list()
    paid=top_install_category['App']['Paid'].to_list()
    free=top_install_category['App']['Free'].to_list()

    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.bar(category, free, color="#f3e151")  
    plt.bar(category, paid, bottom=free, color="#6c3376")

    plt.xlabel('Year')  
    plt.ylabel('Number of futurestud.io Tutorials')
    plt.xticks(rotation=90)
    plt.savefig('hist_type.png',bbox_inches="tight")
